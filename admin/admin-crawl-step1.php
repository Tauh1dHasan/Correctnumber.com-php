...
mysqli_select_db($saha, $database_saha);
$query_selected = "SELECT * FROM settings WHERE settings.crawlRun='Y' ORDER BY settings.id DESC LIMIT $BatchCount";
...
mysqli_select_db($saha, $database_saha);
$query_settings = "SELECT * FROM settings WHERE settings.crawlRun='Y' ORDER BY settings.id DESC LIMIT $BatchCount";
$settings = mysqli_query($saha, $query_settings) or die(mysqli_error($saha));
$row_settings = mysqli_fetch_assoc($settings);
$totalRows_settings = mysqli_num_rows($settings);
...
document.getElementById(pl).innerHTML=document.getElementById(ld).innerHTML;       
 ...
 setTimeout(function() {ajaxFunctionf<?php echo $coluMs2[$i+1];  ?>('load<?php echo $coluMs2[$i+1];  ?>','auto','load<?php echo $coluMs2[$i+1];  ?>','','','admin-crawl-step1-f.php','<?php echo $coluMs2[$i+1];  ?>');},<?php echo $crawl_interval_between_links; ?>);
...
mysqli_select_db($saha, $database_saha);
$query_selectedCrawl = "SELECT * FROM crawl WHERE crawl.actual_url= '$CURL'";
...
?>                 
                                                ...
                                                <tr>
                                                   <td><a href="<?php echo $row_selected['actual_url']; ?>" target="_blank" class="client-link" data-toggle="tab">
<?php echo substr($row_selected['actual_url'],0,20);
	if(strlen($row_selected['actual_url'])>20){echo "...".substr($row_selected['actual_url'],-10);} ?>                                                   
                                                   <?php //echo $row_selected['actual_url']; ?></a>
